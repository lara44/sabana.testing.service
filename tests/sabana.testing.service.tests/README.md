...
tests/sabana.testing.service.tests/
  ...
  Integration/
    ...
    WebApi/
  docs/
    coverage-and-sonarqube-guide.md
```
...
## Pruebas de integracion con SQLite InMemory

Las pruebas de integracion usan SQLite InMemory para validar colaboracion real entre Web API, Application, Infrastructure y EF Core sin requerir PostgreSQL ni Docker.
La configuracion esta embebida en el helper de integracion, usando una conexion SQLite en memoria por ejecucion de prueba.
...
Ejemplo de GitHub Actions (resumen):
...
jobs:
  test-integration:
    ...
    steps:
      ...
      - run: dotnet restore
      - run: dotnet test tests/sabana.testing.service.tests/sabana.testing.service.tests.csproj --filter "FullyQualifiedName~integration"
```

## Guia de cobertura

La documentacion especifica de cobertura y preparacion para SonarQube esta en:

- `docs/coverage-and-sonarqube-guide.md`
